  ...
  describe("getGitHubContext", () => {
    ...
    it("should use GitHub context values", () => {
      ...
      expect(result.jobUrl).toBe(
        "https://github.com/owner/repo/actions/runs/12345/job/test-job",
      );
    ...
    it("should filter sensitive info from malformed URLs", () => {
      process.env.DD_GIT_REPOSITORY_URL =
        "git://user:pass@example.com/repo.git";
    ...
    it("should handle GitHub Enterprise Server URLs", () => {
      ...
      expect(result.pipelineUrl).toBe(
        "https://github.mycompany.com/org/project/actions/runs/999",
      );
      expect(result.jobUrl).toBe(
        "https://github.mycompany.com/org/project/actions/runs/999/job/build",
      );
    });
    ...
    describe("pull request info", () => {
      it("should populate PR info when GITHUB_BASE_REF is set and payload has pull_request", () => {
        ...
        expect(result.pullRequestHeadSha).toBe(
          "df289512a51123083a8e6931dd6f57bb3883d4c4",
        );
        expect(result.pullRequestBaseBranchHeadSha).toBe(
